...
老版本的 skill 写得很爱"refuse"——置信度低就不给结果、没时间戳就不出任务、
没 diarization 就不写草稿。这种"严格"其实是把"安全"当成了拒绝服务的借口。

现在的姿态：**默认把用户要的事先做了**，输出里把每一处不确定都写清楚。
分类置信度只有 0.55 → 仍然出 preset 报告，并附 A–F 让你 redirect；
没 diarization → 用 `SPEAKER_01` / `临时-1` 占位，把"待确认收件人"写进
不确定项；没时间戳 → 用 "录音中段" 引原话简引代替 `mm:ss`。

唯一真正会拦动作的是 **对外发送**：草稿编辑好后等你点 `发送` 按钮才出去。
这是在 UI 层守的，不是在模型层守的。
...
这个 skill 的默认姿态是 **serve, then surface** —— **先把用户要的活干出来**，
再把不确定的地方标清楚。元数据缺了不是拒绝输出的理由，是降级标注的理由。
用户才是 human-in-the-loop，skill 的工作是把"录音里说了什么"和"我推测了什么"
分开列清，而不是替用户做决定。
...
| 规则 | 为什么 |
| --- | --- |
| **对外发送必须显式 confirm** | 草稿生成 → 用户审查 → 用户点 `发送` → 才出去。模型不能"决定发了"，因为发出去是不可逆的 |

下面四条是**诚实规则** —— 它们规范输出怎么写，**不阻断**输出本身：

| 规则 | 怎么落实 |
| --- | --- |
| 不静默编造身份 | 不知道叫谁就用 `SPEAKER_01` / `临时-1`，并在不确定项里标"待确认收件人" |
| 推测必标注 | 没说的截止时间、推断的归属 → `（推测）` / `（待确认）` |
| 不在个人草稿里默认夹带全文 | 只引相关 excerpt；用户主动要全文再给 |
| API key 走环境变量 | `settings.yaml` 写 `api_key_env: ASR_API_KEY`，不内联、不回显 |
